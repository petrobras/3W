...
class TestParquetDataset:
    ...
    def test_full_loading(self, parquet_dataset_path):
        ...
        for e in dataset:
            assert list(e["signal"].columns) == _SIGNAL_NAMES + [_LABEL_NAME] # _LABEL_NAME maps to signals
    ...
    def test_file_splitting(self, parquet_dataset_path):
        ...
        flist   = ["0/WELL_test.parquet", "1/DRAWN_test.parquet", "3/SIMULATED_test.parquet"]
        config  = DatasetConfig(path=parquet_dataset_path, file_type="parquet", target_column=_LABEL_NAME, split="list",
                                file_list=flist)
        dataset = ParquetDataset(config)
    ...
    def test_event_filtering(self, parquet_dataset_path):
        ...
        """
        event_type = ["real"]
        config  = DatasetConfig(path=parquet_dataset_path, file_type="parquet", target_column=_LABEL_NAME, split=None,
