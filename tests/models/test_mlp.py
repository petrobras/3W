...
class TestMLPConfig:
    ...
    def test_invalid_input_size_zero(self):
        ...
        with pytest.raises(ValueError, match="input_size.*must be > 0"):
    ...
    def test_invalid_hidden_sizes_negative(self):
        ...
        with pytest.raises(ValueError, match="hidden_sizes must be a tuple"):
...
class TestMLP:
    ...
    def test_model_creation_without_input_size(self, dynamic_config):
        """Model without input_size should defer layer building."""
    ...
    def test_forward_pass_dynamic_input(self, dynamic_config):
        """Forward pass should build layers dynamically."""
        model = MLP(dynamic_config)
        x = torch.randn(8, 50)
        output = model(x)
        assert output.shape == (8, 3)
    ...
    def test_get_params_without_built_model(self, dynamic_config):
        """get_params should return dummy param when model not built."""
        model = MLP(dynamic_config)
        params = list(model.get_params())
        assert len(params) == 1
