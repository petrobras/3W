...
class PerClassAnomalyVisualizer:
    ...
    def __init__(self):
        self.default_colors = ['red', 'orange', 'brown', 'pink', 'purple', 'olive', 'cyan', 'magenta']
    
    def create_comprehensive_analysis(self, normal_errors, anomaly_errors, per_class_metrics, threshold):
        """
        ...
        self._plot_error_distributions(axes[0, 0], normal_errors, anomaly_errors, per_class_metrics, threshold)
        ...
        self._plot_error_boxplots(axes[1, 2], normal_errors, anomaly_errors, per_class_metrics)
    ...
    def _plot_error_distributions(self, ax, normal_errors, anomaly_errors, per_class_metrics, threshold):
        """Plot error distributions for normal and per-class anomalies."""
        ax.hist(normal_errors, bins=30, alpha=0.6, label='Normal', color='blue', density=True)
        ...
        for cls, metrics in per_class_metrics.items():
            ...
            if 'indices' in metrics:
                ...
                ax.hist(cls_errors, bins=20, alpha=0.6, 
                       label=f'Class {cls}', color=metrics['color'], density=True)
    ...
    def _plot_error_amplification(self, ax, normal_errors, per_class_metrics):
        ...
        classes = list(per_class_metrics.keys())
        amplifications = [metrics['mean_error'] / normal_mean for metrics in per_class_metrics.values()]
        colors = [metrics['color'] for metrics in per_class_metrics.values()]
        
        bars = ax.bar(classes, amplifications, color=colors, alpha=0.7, edgecolor='black')
    ...
    def _plot_error_boxplots(self, ax, normal_errors, anomaly_errors, per_class_metrics):
