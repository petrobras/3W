...
class AdvancedClusteringVisualizer:
    ...
    def _plot_performance_radar_chart(self, clustering_data, advanced_results):
        ...
        for alg, name in zip(algorithms, algorithm_names):
            if alg in advanced_results and advanced_results[alg] is not None:
                ...
                if alg == "optimized_dbscan":
                    ...
                    actual_clusters = result.get(
                        "k", result.get("n_components", result.get("n_clusters", 3))
                    )

                cluster_score = 1 - abs(actual_clusters - ideal_clusters) / max(
                    ideal_clusters, actual_clusters
                )
        ...
        if metrics_data:
            ...
            for i, (metrics, label, color) in enumerate(
                zip(metrics_data, algorithm_labels, colors)
            ):
            ...
            ax.set_ylim(0, 1)
            ax.set_title(
                "Clustering Algorithm Performance Comparison", fontweight="bold", pad=20
            )
    ...
    def _plot_stability_analysis(self, clustering_data, advanced_results):
        ...
        if (
            "enhanced_kmeans" in advanced_results
            and advanced_results["enhanced_kmeans"] is not None
        ):
            ...
            # Stability over runs
            ax1.plot(
                range(1, n_runs + 1), stability_scores, "o-", linewidth=2, markersize=8
            )
            ax1.axhline(
                y=np.mean(stability_scores),
                color="red",
                linestyle="--",
            ...
            plt.suptitle(
                f"Clustering Stability Analysis (K={best_k})", fontweight="bold"
            )
            plt.tight_layout()
            plt.show()
