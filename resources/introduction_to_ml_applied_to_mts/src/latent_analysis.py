...
class LatentSpaceAnalyzer:
    ...
    def extract_latent_representations(self, autoencoder, normal_scaled, anomaly_scaled, 
                                     max_normal_samples=800, max_anomaly_samples=400):
        """
        ...
        print(f"Processing {normal_sample_size} normal and {anomaly_sample_size} anomaly samples...")
        
        # Extract latent representations
        self.normal_latent = self.encoder.predict(normal_scaled[:normal_sample_size], verbose=0)
        self.anomaly_latent = self.encoder.predict(anomaly_scaled[:anomaly_sample_size], verbose=0)
    ...
    def compute_latent_statistics(self, per_class_metrics, anomaly_classes, max_samples=400):
        """
        ...
        for cls in sorted(per_class_metrics.keys()):
            ...
            cls_indices = [i for i, label in enumerate(anomaly_class_subset) 
                          if str(int(label) if isinstance(label, (np.integer, int)) else label) == cls]
    ...
    def create_comprehensive_latent_visualization(self, per_class_metrics, class_latent_stats, 
                                                normal_tsne, anomaly_tsne, anomaly_classes):
        """
        ...
        self._plot_tsne_perclass(axes[0, 0], normal_tsne, anomaly_tsne, per_class_metrics, anomaly_classes)
        ...
        self._plot_mean_latent_comparison(axes[1, 0], class_latent_stats, per_class_metrics)
        
        # 6. Latent space quality vs detection accuracy
        self._plot_separation_vs_accuracy(axes[1, 1], class_latent_stats, per_class_metrics)
        
        # 7. Most discriminative dimension distribution
        self._plot_discriminative_dimension(axes[1, 2], class_latent_stats, per_class_metrics)
        
        # 8. t-SNE vs Detection Performance correlation
        self._plot_tsne_vs_performance(axes[1, 3], normal_tsne, anomaly_tsne, 
                                     per_class_metrics, anomaly_classes)
    ...
    def _plot_tsne_perclass(self, ax, normal_tsne, anomaly_tsne, per_class_metrics, anomaly_classes):
        """Plot t-SNE with per-class colors."""
        # Plot normal data
        ax.scatter(normal_tsne[:, 0], normal_tsne[:, 1],
        ...
        for cls in sorted(per_class_metrics.keys()):
            cls_mask = np.array([str(int(label) if isinstance(label, (np.integer, int)) else label) == cls 
                                for label in anomaly_classes[:len(anomaly_tsne)]])
            if np.any(cls_mask):
                cls_tsne = anomaly_tsne[cls_mask]
                color = per_class_metrics[cls]['color']
                
                ax.scatter(cls_tsne[:, 0], cls_tsne[:, 1],
    ...
    def _plot_raw_latent_space(self, ax, class_latent_stats, per_class_metrics):
        ...
        if self.normal_latent.shape[1] >= 2:
            ax.scatter(self.normal_latent[:, 0], self.normal_latent[:, 1],
            ...
            for cls in sorted(per_class_metrics.keys()):
                if cls in class_latent_stats:
                    ...
                    color = per_class_metrics[cls]['color']
                    ax.scatter(cls_latent[:, 0], cls_latent[:, 1],
    ...
    def _plot_class_separation(self, ax, class_latent_stats, per_class_metrics):
        ...
        for bar, sep in zip(bars, separations):
            ...
            ax.text(bar.get_x() + bar.get_width()/2., height + max(separations)*0.01,
    ...
    def _plot_mean_latent_comparison(self, ax, class_latent_stats, per_class_metrics):
        ...
        ax.plot(dims, normal_mean, 'o-', color='blue', label='Normal Mean', 
        ...
        for cls in sorted(class_latent_stats.keys()):
            if cls in class_latent_stats:
                ...
                color = per_class_metrics[cls]['color']
                ax.plot(dims, cls_mean, 's-', color=color, 
    ...
    def _plot_separation_vs_accuracy(self, ax, class_latent_stats, per_class_metrics):
        ...
        ax.scatter(separations, accuracies, c=colors, s=100, alpha=0.7, edgecolors='black')
        
        # Add class labels
        for i, cls in enumerate(classes):
            ax.annotate(f'Class {cls}', (separations[i], accuracies[i]),
    ...
    def _plot_discriminative_dimension(self, ax, class_latent_stats, per_class_metrics):
        ...
        ax.hist(self.normal_latent[:, most_disc_dim], bins=20, alpha=0.5, 
        ...
        for cls in sorted(class_latent_stats.keys()):
            if cls in class_latent_stats:
                cls_latent = class_latent_stats[cls]['latent_data']
                color = per_class_metrics[cls]['color']
                ax.hist(cls_latent[:, most_disc_dim], bins=15, alpha=0.6,
    ...
    def _plot_tsne_vs_performance(self, ax, normal_tsne, anomaly_tsne, per_class_metrics, anomaly_classes):
        ...
        ax.scatter(tsne_separations, accuracies, c=colors, s=100, alpha=0.7, edgecolors='black')
        
        for i, cls in enumerate(classes):
            ax.annotate(f'Class {cls}', (tsne_separations[i], accuracies[i]),
                       xytext=(5, 5), textcoords='offset points', fontsize=9)
