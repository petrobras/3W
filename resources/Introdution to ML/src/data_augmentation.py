...
class DataAugmentor:
    ...
    def add_noise(self, dfs: List[pd.DataFrame], classes: List[str],
                  noise_type: str = 'gaussian', noise_level: float = 0.01,
                  target_columns: Optional[List[str]] = None) -> Tuple[List[pd.DataFrame], List[str]]:
        """
            ...
            for col in numeric_cols:
                if col in df.columns:
                    ...
                    if noise_type == 'gaussian':
                        ...
                        noise = np.random.uniform(-noise_range, noise_range, len(signal))
                    elif noise_type == 'sensor_specific':
                        # Different noise levels for different sensor types
                        if 'P-TPT' in col:  # Pressure sensor
                            noise = np.random.normal(0, noise_level * signal_std * 0.8, len(signal))
                        elif 'T-TPT' in col:  # Temperature sensor
                            noise = np.random.normal(0, noise_level * signal_std * 1.2, len(signal))
                        else:
                            noise = np.random.normal(0, noise_level * signal_std, len(signal))
    ...
    def random_undersample(self, dfs: List[pd.DataFrame], classes: List[str],
                          target_class_counts: Optional[Dict[str, int]] = None,
                          strategy: str = 'auto') -> Tuple[List[pd.DataFrame], List[str]]:
        """
        ...
        if target_class_counts is None:
            if strategy == 'auto':
                ...
                target_class_counts = {cls: min(count, int(median_count)) 
    ...
    def random_oversample(self, dfs: List[pd.DataFrame], classes: List[str],
                         target_class_counts: Optional[Dict[str, int]] = None,
                         strategy: str = 'auto') -> Tuple[List[pd.DataFrame], List[str]]:
        """
        ...
        if target_class_counts is None:
            if strategy == 'auto':
                ...
                median_count = np.median(list(class_counts.values()))
                target_class_counts = {cls: max(count, int(median_count)) 
                                     for cls, count in class_counts.items()}
    ...
    def smote_like_oversample(self, dfs: List[pd.DataFrame], classes: List[str],
                             k_neighbors: int = 5, 
                             target_class_counts: Optional[Dict[str, int]] = None) -> Tuple[List[pd.DataFrame], List[str]]:
        """
        SMOTE-like oversampling for time series data.
                    ...
                    for _ in range(needed_samples):
                        ...
                        for col in numeric_cols:
                            if col in base_df.columns and col in neighbor_df.columns:
                                ...
                                synthetic_values = (1 - alpha) * base_values + alpha * neighbor_values
    ...
    def balance_classes(self, dfs: List[pd.DataFrame], classes: List[str],
                       strategy: str = 'combined', 
                       target_samples_per_class: Optional[int] = None) -> Tuple[List[pd.DataFrame], List[str]]:
        """
        ...
        if strategy == 'undersample':
            ...
            return self.smote_like_oversample(dfs, classes, target_class_counts=target_counts)
        elif strategy == 'combined':
            # First undersample majority classes
            balanced_dfs, balanced_classes = self.random_undersample(dfs, classes, target_counts, 'auto')
            # Then oversample minority classes
            return self.random_oversample(balanced_dfs, balanced_classes, target_counts, 'auto')
        
        return dfs, classes
    
    def create_augmentation_pipeline(self, dfs: List[pd.DataFrame], classes: List[str],
                                   config: Dict) -> Tuple[List[pd.DataFrame], List[str]]:
        """
        ...
        if config.get('add_noise', False):
            ...
            augmented_dfs, augmented_classes = self.add_noise(
                augmented_dfs, augmented_classes,
        ...
        if self.verbose:
            ...
            print(f"   • Augmentation factor: {len(augmented_classes)/len(classes):.2f}x")
        
        return augmented_dfs, augmented_classes


# Convenience functions for common use cases
def quick_noise_augmentation(dfs: List[pd.DataFrame], classes: List[str], 
                           noise_level: float = 0.01) -> Tuple[List[pd.DataFrame], List[str]]:
