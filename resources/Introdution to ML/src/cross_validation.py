...
class CrossValidator:
    ...
    def separate_real_simulated_data(self, dfs: List, classes: List, 
                                   loader = None, 
                                   fallback_real_proportion: float = 0.7) -> Tuple[List, List, List, List]:
        """
        ...
        try:
            ...
            if loader and hasattr(loader, 'get_separated_data'):
                ...
                real_proportion = total_real_original / (total_real_original + total_sim_original)
                ...
                if self.verbose:
                    ...
                    print(f"   Original - Real: {total_real_original}, Simulated: {total_sim_original}")
                    print(f"   Processed - Real: {len(real_dfs)}, Simulated: {len(simulated_dfs)}")
            ...
            simulated_classes = classes[real_cutoff:] if real_cutoff < total_samples else []
    ...
    def create_cv_folds(self, real_dfs: List, real_classes: List, 
                       simulated_dfs: List, simulated_classes: List) -> List[Dict[str, Any]]:
        """
        ...
        if len(real_dfs) < self.n_folds:
            raise ValueError(f"Not enough real data samples for {self.n_folds}-fold cross-validation. "
        ...
        if min_class_count < self.n_folds:
            if self.verbose:
                print(f"⚠️ Warning: Some classes have fewer than {self.n_folds} samples.")
        ...
        for fold_idx, (train_real_idx, test_real_idx) in enumerate(splits):
            ...
            fold_train_classes = [real_classes[i] for i in train_real_idx] + simulated_classes
    ...
    def print_cv_summary(self, cv_folds: List[Dict[str, Any]], 
                        real_dfs: List, simulated_dfs: List) -> None:
        """
        ...
        print(f"   • Avg simulated samples for training: {total_sim_samples} (constant)")
        print(f"   • Avg real samples for testing: {avg_test_real:.1f}")
        print(f"   • Avg train/test ratio: {avg_train_real/total_real_samples*100:.1f}% / {avg_test_real/total_real_samples*100:.1f}%")
        ...
        print(f"   • Test coverage: {len(all_test_indices)} total assignments across folds")
        print(f"   • Unique test samples: {len(unique_test_indices)}")
        print(f"   • Each real sample tested exactly once: {'✅' if len(unique_test_indices) == total_real_samples else '❌'}")
        
        print(f"\n💾 Variables Created:")
        print(f"   • cv_folds: List of {len(cv_folds)} cross-validation fold dictionaries")
        print(f"   • Each fold contains train_dfs, test_dfs, train_classes, test_classes")
        
        print(f"\n🎯 Ready for {len(cv_folds)}-fold cross-validation training and evaluation!")
    
    def get_fold_data(self, fold_number: int, cv_folds: List[Dict[str, Any]]) -> Tuple[List, List, List, List]:
        """
        ...
        return (fold['train_dfs'], fold['train_classes'], 
                fold['test_dfs'], fold['test_classes'])
    
    def print_fold_summary(self, fold_number: int, cv_folds: List[Dict[str, Any]]) -> None:
        """
    ...
    def compare_all_folds(self, cv_folds: List[Dict[str, Any]]) -> None:
        ...
        print(f"{'Fold':<6} {'Train Real':<12} {'Train Sim':<12} {'Test Real':<12} {'Train%':<10} {'Test%':<10}")
        print("-" * 70)
        ...
        for i, fold in enumerate(cv_folds):
            ...
            print(f"{fold_num:<6} {train_real:<12} {train_sim:<12} {test_real:<12} {train_pct:<9.1f}% {test_pct:<9.1f}%")
    
    def setup_complete_cv(self, dfs: List, classes: List, 
                         loader = None) -> Tuple[List[Dict[str, Any]], Tuple[List, List, List, List]]:
        """
        ...
        real_dfs, real_classes, simulated_dfs, simulated_classes = self.separate_real_simulated_data(
            dfs, classes, loader)
        
        # Step 2: Create cross-validation folds
        cv_folds = self.create_cv_folds(real_dfs, real_classes, simulated_dfs, simulated_classes)
...
def get_fold_data(fold_number: int, cv_folds: List[Dict[str, Any]]) -> Tuple[List, List, List, List]:
