...
class AnomalyDetector:
    ...
    def compute_reconstruction_errors(self, autoencoder, normal_data, anomaly_data):
        ...
        print(f"   • Normal error range: [{np.min(self.normal_errors):.6f}, {np.max(self.normal_errors):.6f}]")
        print(f"   • Anomaly error range: [{np.min(self.anomaly_errors):.6f}, {np.max(self.anomaly_errors):.6f}]")
    ...
    def determine_threshold(self, method='percentile', percentile=95):
        ...
        if self.normal_errors is None:
            raise ValueError("Normal errors not computed. Call compute_reconstruction_errors first.")
    ...
    def evaluate_detection(self):
        ...
        # Calculate metrics
        normal_correct = np.sum(normal_predictions == 0)  # Should be classified as normal (0)
        normal_total = len(normal_predictions)
        normal_accuracy = normal_correct / normal_total
        
        anomaly_correct = np.sum(anomaly_predictions == 1)  # Should be classified as anomaly (1)
        anomaly_total = len(anomaly_predictions)
        anomaly_accuracy = anomaly_correct / anomaly_total
        
        overall_accuracy = (normal_correct + anomaly_correct) / (normal_total + anomaly_total)
        ...
        print(f"   • Normal data accuracy: {normal_accuracy:.3f} ({normal_correct}/{normal_total})")
        print(f"   • Anomaly data accuracy: {anomaly_accuracy:.3f} ({anomaly_correct}/{anomaly_total})")
        print(f"   • Overall accuracy: {overall_accuracy:.3f}")
        print(f"   • False positives (normal as anomaly): {normal_total - normal_correct}")
        print(f"   • False negatives (anomaly as normal): {anomaly_total - anomaly_correct}")
    ...
    def plot_error_distributions(self, figsize=(12, 8)):
        ...
        if self.normal_errors is None or self.anomaly_errors is None:
            raise ValueError("Errors not computed. Call compute_reconstruction_errors first.")
        
        fig, axes = plt.subplots(2, 2, figsize=figsize)
        
        # Histogram comparison
        axes[0, 0].hist(self.normal_errors, bins=50, alpha=0.7, label='Normal', color='blue', density=True)
        axes[0, 0].hist(self.anomaly_errors, bins=50, alpha=0.7, label='Anomaly', color='red', density=True)
        if self.threshold is not None:
            axes[0, 0].axvline(self.threshold, color='green', linestyle='--', 
        ...
        if self.threshold is not None:
            axes[0, 1].axhline(self.threshold, color='green', linestyle='--', 
                              label=f'Threshold: {self.threshold:.6f}')
        ...
        anomaly_indices = range(len(self.normal_errors), len(self.normal_errors) + len(self.anomaly_errors))
        
        axes[1, 0].scatter(normal_indices, self.normal_errors, alpha=0.6, label='Normal', color='blue', s=20)
        axes[1, 0].scatter(anomaly_indices, self.anomaly_errors, alpha=0.6, label='Anomaly', color='red', s=20)
        if self.threshold is not None:
            axes[1, 0].axhline(self.threshold, color='green', linestyle='--', 
                              label=f'Threshold: {self.threshold:.6f}')
        ...
        # Log scale histogram
        axes[1, 1].hist(self.normal_errors, bins=50, alpha=0.7, label='Normal', color='blue', density=True)
        axes[1, 1].hist(self.anomaly_errors, bins=50, alpha=0.7, label='Anomaly', color='red', density=True)
        if self.threshold is not None:
            axes[1, 1].axvline(self.threshold, color='green', linestyle='--', 
                              label=f'Threshold: {self.threshold:.6f}')
    ...
    def plot_roc_curve(self):
        ...
        y_true = np.concatenate([np.zeros(len(self.normal_errors)), 
                                np.ones(len(self.anomaly_errors))])
...
def visualize_latent_space(autoencoder, normal_data, anomaly_data, n_samples=500):
    ...
    latent_model = tf.keras.Model(inputs=autoencoder.model.input, outputs=encoder.output)
    ...
    labels = np.concatenate([np.zeros(len(normal_latent)), np.ones(len(anomaly_latent))])
