...
class DataPersistence:
    ...
    def save_cv_folds(self, cv_folds: List[Dict[str, Any]], 
        ...
        for fold_idx, fold_info in enumerate(cv_folds):
            ...
            if self.verbose:
                ...
                print(f"      • Training: {len(fold_info['train_dfs'])} samples → {train_file}")
                print(f"      • Testing: {len(fold_info['test_dfs'])} samples → {test_file}")
    ...
    def save_windowed_data(self, windowing_results: Dict[str, Any],
                          save_format: str = 'pickle',
                          compression: str = 'snappy') -> str:
        """
        ...
        if 'all_windowed_folds' in windowing_results:
            return self._save_all_windowed_folds(windowing_results, save_format, compression)
        
        # Original format - save single fold windowed data
        return self._save_single_fold_windowed_data(windowing_results, save_format, compression)
    
    def _save_all_windowed_folds(self, windowing_results: Dict[str, Any],
                                save_format: str = 'pickle',
                                compression: str = 'snappy') -> str:
        ...
        if self.verbose:
            print(f"💾 Saving windowed data for {len(all_windowed_folds)} cross-validation folds")
        ...
        for i, fold_data in enumerate(all_windowed_folds):
            ...
            fold_metadata_file = os.path.join(fold_windowed_dir, 'windowing_metadata.json')
            ...
            if self.verbose:
                ...
                print(f"   • Fold {fold_num}: {train_count} train, {test_count} test windows")
        ...
        overall_metadata_file = os.path.join(windowed_dir, 'overall_windowing_summary.json')
        with open(overall_metadata_file, 'w') as f:
            json.dump(overall_metadata, f, indent=2)
        
        if self.verbose:
            total_windows = windowing_results.get('total_train_windows', 0) + windowing_results.get('total_test_windows', 0)
            print(f"   ✅ Total: {total_windows} windows across {len(all_windowed_folds)} folds")
            print(f"   📊 Summary: {overall_metadata_file}")
        
        return windowed_dir
    
    def _save_single_fold_windowed_data(self, windowing_results: Dict[str, Any],
                                       save_format: str = 'pickle',
                                       compression: str = 'snappy') -> str:
        ...
        if self.verbose:
            print(f"   • Training windows: {len(single_fold_data['train_windowed_dfs'])} → {train_windowed_file}")
            print(f"   • Test windows: {len(single_fold_data['test_windowed_dfs'])} → {test_windowed_file}")
            print(f"   • Metadata: {metadata_file}")
    ...
    def _save_dataframes(self, dfs: List[pd.DataFrame], classes: List[str], 
                        filepath: str, save_format: str, compression: str) -> str:
        """
    ...
    def _save_as_pickle(self, dfs: List[pd.DataFrame], classes: List[str], 
                        filepath: str, compression: str) -> str:
    ...
    def _save_as_csv(self, dfs: List[pd.DataFrame], classes: List[str], filepath: str) -> str:
        """Save as CSV format."""
        combined_data = []
        for i, (df, class_label) in enumerate(zip(dfs, classes)):
            df_copy = df.copy()
    ...
    def _save_as_pickle(self, dfs: List[pd.DataFrame], classes: List[str], filepath: str) -> str:
    ...
    def load_cv_folds(self, fold_dir: Optional[str] = None, 
                     save_format: str = 'pickle') -> List[Dict[str, Any]]:
        """
    ...
    def _load_dataframes(self, filepath: str, save_format: str) -> Tuple[List[pd.DataFrame], List[str]]:
    ...
    def _load_all_windowed_folds(self, windowed_dir: str) -> Dict[str, Any]:
        ...
        overall_summary_file = os.path.join(windowed_dir, 'overall_windowing_summary.json')
        ...
        for fold_detail in overall_summary['fold_details']:
            ...
            if os.path.exists(fold_windowed_dir):
                ...
                metadata_file = os.path.join(fold_windowed_dir, 'windowing_metadata.json')
        ...
        if self.verbose:
            ...
            print(f"   📊 Total: {total_train_windows} train, {total_test_windows} test windows")
    ...
    def _load_single_fold_windowing(self, windowed_dir: str) -> Dict[str, Any]:
        ...
        if os.path.exists(stats_file):
            with open(stats_file, 'r') as f:
                ...
                windowing_results['train_statistics'] = metadata.get('train_statistics', {})
                windowing_results['test_statistics'] = metadata.get('test_statistics', {})
        
        if self.verbose:
            train_count = len(windowing_results.get('train_windowed_dfs', []))
            test_count = len(windowing_results.get('test_windowed_dfs', []))
            print(f"   ✅ Loaded legacy windowing: {train_count} train, {test_count} test windows")
...
def save_complete_pipeline(cv_folds: List[Dict[str, Any]], 
                          windowing_results: Dict[str, Any],
                          processing_config: Dict[str, Any],
                          base_dir: str = 'processed_data') -> str:
    """
    ...
    # Save windowed data
    windowed_dir = persistence.save_windowed_data(windowing_results, save_format='pickle')
    ...
    if 'all_windowed_folds' in windowing_results:
        ...
        summary_data['windowed_folds_count'] = len(windowing_results['all_windowed_folds'])
        summary_data['total_train_windows'] = windowing_results.get('total_train_windows', 0)
        summary_data['total_test_windows'] = windowing_results.get('total_test_windows', 0)
        summary_data['windowing_applied_to_all_folds'] = True
    else:
        # Legacy single-fold structure
        summary_data['windowed_train_count'] = len(windowing_results.get('train_windowed_dfs', []))
        summary_data['windowed_test_count'] = len(windowing_results.get('test_windowed_dfs', []))
...
def load_complete_pipeline(base_dir: str = 'processed_data') -> Dict[str, Any]:
    ...
    results = {
        ...
        'metadata': {
            ...
            'total_windowed_folds': len(windowing_results.get('all_windowed_folds', [])),
            'total_train_windows': windowing_results.get('total_train_windows', 0),
            'total_test_windows': windowing_results.get('total_test_windows', 0),
