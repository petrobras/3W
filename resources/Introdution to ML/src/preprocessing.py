...
class DataPreprocessor:
    ...
    def apply_scaling(self, dfs: List[pd.DataFrame], classes: List[str], 
                     method: str = 'minmax', exclude_columns: List[str] = None) -> Tuple[List[pd.DataFrame], List[str]]:
        """
        ...
        if method not in self.scalers:
            raise ValueError(f"Unknown scaling method: {method}. Choose from {list(self.scalers.keys())}")
        ...
        self._print_scaling_summary(total_processed, successfully_scaled, skipped_dfs, scaled_classes, method)
    ...
    def _scale_single_dataframe(self, df: pd.DataFrame, scaler, method: str, 
                               exclude_columns: List[str], df_index: int, 
                               class_label: str) -> Optional[pd.DataFrame]:
        """
        ...
        if len(numeric_data) == 0:
            if df_index <= 5:
                print(f"⚠️  DataFrame {df_index}: No valid numeric data after removing NaN, skipping...")
            return None
    ...
    def _print_scaling_summary(self, total_processed: int, successfully_scaled: int, 
                              skipped_dfs: int, scaled_classes: List[str], method: str) -> None:
    ...
    def get_scaling_statistics(self, scaled_dfs: List[pd.DataFrame], method: str = 'minmax') -> Dict[str, Any]:
        """
    ...
    def select_random_dataframe(self, dfs: List[pd.DataFrame], classes: List[str], 
                               required_columns: List[str] = None, min_samples: int = 100,
                               seed: int = 42) -> Tuple[int, pd.DataFrame, str, int]:
        """
        ...
        if not available_dfs:
            raise ValueError(f"No dataframes found with required columns {required_columns} and {min_samples}+ samples")
    ...
    def apply_multiple_scalers(self, data: pd.DataFrame, exclude_columns: List[str] = None) -> Dict[str, pd.DataFrame]:
        """
    ...
    def validate_scaling(self, scaled_df: pd.DataFrame, method: str = 'minmax') -> Dict[str, bool]:
        """
        ...
        for col in numeric_cols:
            ...
            if method == 'minmax':
                ...
                validation['correct_scaling'][col] = (col_data.min() >= -0.001 and col_data.max() <= 1.001)
            elif method == 'standard':
                # Standard should have mean ≈ 0, std ≈ 1
                validation['correct_scaling'][col] = (abs(col_data.mean()) < 0.1 and abs(col_data.std() - 1) < 0.1)
            elif method == 'robust':
                ...
                validation['correct_scaling'][col] = True  # Complex validation for normalizer
    ...
    def split_train_test_by_class(
        ...
        for class_id, df in dfs.items():
            ...
            if target_features is not None:
                ...
                if not available_features:
                    print(f"Warning: No target features found in class {class_id}. Skipping...")
                    continue
        ...
        if train_dfs and test_dfs:
            ...
            test_combined = test_combined.sample(frac=1, random_state=random_state + 1000).reset_index(drop=True)
            
            # Update features used
            split_stats['features_used'] = [col for col in train_combined.columns if col != 'class']
    ...
    def analyze_split_distribution(
        ...
        if show_details:
            ...
            print(f"Feature Names: {split_stats['features_used'][:5]}{'...' if len(split_stats['features_used']) > 5 else ''}")
            print("\nClass Distribution:")
    ...
    def validate_split_balance(
        ...
        for class_id in common_classes:
            ...
            validation_results['max_difference'] = max(validation_results['max_difference'], difference)
        
        return validation_results
    
    def train_test_split_dataframes(self, dfs: List[pd.DataFrame], classes: List[str], 
        ...
        print(f"   Train samples: {len(train_dfs)} dataframes ({len(train_dfs)/len(dfs)*100:.1f}%)")
        print(f"   Test samples: {len(test_dfs)} dataframes ({len(test_dfs)/len(dfs)*100:.1f}%)")
        
        print(f"\n🎯 Class Distribution:")
        print(f"{'Class':<8} {'Original':<10} {'Train':<8} {'Test':<8} {'Train%':<8} {'Test%':<8}")
        print("-" * 50)
        ...
        for class_label in sorted(original_class_counts.keys()):
            ...
            print(f"{class_label:<8} {orig_count:<10} {train_count:<8} {test_count:<8} {train_pct:<7.1f}% {test_pct:<7.1f}%")
        
        return results
    
    def validate_dataframe_split_balance(self, split_results: Dict[str, Any], tolerance: float = 0.05) -> Dict[str, bool]:
        """
        ...
        for class_label in original_dist.keys():
            ...
            train_balanced = abs(actual_train_ratio - validation['expected_train_ratio']) <= tolerance
            test_balanced = abs(actual_test_ratio - validation['expected_test_ratio']) <= tolerance
            ...
            print(f"{status} Class {class_label}: Train {actual_train_ratio:.3f} (exp: {validation['expected_train_ratio']:.3f}), "
    ...
    def create_time_windows(self, dfs: List[pd.DataFrame], classes: List[str], 
                           window_size: int = 300, stride: int = None, 
                           min_window_size: int = None) -> Tuple[List[pd.DataFrame], List[str], List[Dict]]:
        """
        ...
        for i, (df, class_label) in enumerate(zip(dfs, classes)):
            ...
            if df_length < min_window_size:
                skipped_samples += 1
                print(f"⚠️  Skipped sample {i+1} (class {class_label}): too small ({df_length} < {min_window_size})")
                continue
            ...
            if sample_windows > 0:
                print(f"✅ Sample {i+1} (class {class_label}): {sample_windows} windows from {df_length} points")
        ...
        print(f"   Average windows per sample: {total_windows/(len(dfs) - skipped_samples):.1f}")
    ...
    def get_windowing_statistics(self, window_metadata: List[Dict]) -> Dict[str, Any]:
        ...
        sample_window_counts = Counter([meta['original_sample_id'] for meta in window_metadata])
    ...
    def apply_windowing_to_split(self, train_dfs: List[pd.DataFrame], test_dfs: List[pd.DataFrame],
                                train_classes: List[str], test_classes: List[str],
                                window_size: int = 300, stride: int = None,
                                min_window_size: int = None) -> Dict[str, Any]:
        """
        ...
        train_windowed_dfs, train_windowed_classes, train_metadata = self.create_time_windows(
            train_dfs, train_classes, window_size, stride, min_window_size
        )
        
        # Window test data
        print("\n🧪 Processing test data:")
        test_windowed_dfs, test_windowed_classes, test_metadata = self.create_time_windows(
        ...
        result = {
            ...
            'windowing_parameters': {
                ...
                'min_window_size': min_window_size if min_window_size is not None else window_size
            }
        }
