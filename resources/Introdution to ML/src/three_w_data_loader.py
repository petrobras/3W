...
class ThreeWDataLoader:
    ...
    def discover_folds(self) -> List[str]:
        ...
        if not os.path.exists(self.windowed_base_dir):
            raise FileNotFoundError(f"Windowed directory not found: {self.windowed_base_dir}")
    ...
    def load_fold_data(self, fold_dir: str, load_train: bool = True, 
                      load_test: bool = True, max_windows: Optional[int] = None) -> Dict:
        """
    ...
    def _load_data_type(self, fold_path: str, data_type: str, 
                       max_windows: Optional[int] = None) -> Optional[Tuple[List, List]]:
        """
        ...
        pickle_file = os.path.join(fold_path, f'{data_type}_windowed.{self.config.SAVE_FORMAT}')
        ...
        if os.path.exists(pickle_file):
            ...
            try:
                data_dfs, data_classes = self.persistence._load_from_parquet(parquet_file)
            except Exception as e:
    ...
    def load_all_folds(self, load_all_folds: bool = True, specific_folds: List[int] = None,
                      load_train_data: bool = True, load_test_data: bool = True,
                      max_windows_per_fold: Optional[int] = None) -> Dict:
        """
        ...
        if load_all_folds:
            ...
            folds_to_process = [f'fold_{i}' for i in specific_folds if f'fold_{i}' in fold_dirs]
        
        print(f"🎯 Processing {len(folds_to_process)} folds: {', '.join(folds_to_process)}")
    ...
    def get_fold_data(self, fold_name: str, data_type: str = 'both') -> Optional[Union[Dict, Tuple]]:
        """
    ...
    def get_class_distribution(self, data_type: str = 'both', by_fold: bool = False) -> Dict:
        """
        ...
        if by_fold:
            ...
            for fold_name, fold_data in self.all_fold_data.items():
                ...
                if data_type in ['train', 'both'] and fold_data['train_data'] is not None:
    ...
    def estimate_memory_usage(self) -> Dict[str, Union[int, float]]:
        ...
        total_windows = (self.loading_stats['total_train_windows'] + 
    ...
    def print_summary(self):
        ...
        # Basic statistics
        print(f"✅ Successfully processed folds: {self.loading_stats['successful_folds']}")
        print(f"❌ Failed folds: {self.loading_stats['failed_folds']}")
        print(f"🚄 Total training windows: {self.loading_stats['total_train_windows']:,}")
        print(f"🧪 Total testing windows: {self.loading_stats['total_test_windows']:,}")
        print(f"🎯 Total windows: {self.loading_stats['total_train_windows'] + self.loading_stats['total_test_windows']:,}")
        ...
        if self.loading_stats['successful_folds'] > 0:
            avg_load_time = self.loading_stats['total_load_time'] / self.loading_stats['successful_folds']
            print(f"\n⚡ Performance Summary:")
            print(f"   • Average fold load time: {avg_load_time:.2f}s")
            
            if self.loading_stats['total_load_time'] > 0:
                windows_per_sec = ((self.loading_stats['total_train_windows'] + 
                                  self.loading_stats['total_test_windows']) / 
        ...
        if 'error' not in memory_stats:
            ...
            print(f"   • Estimated memory usage: {memory_stats['total_memory_mb']:.1f} MB")
...
def load_complete_dataset(windowed_base_dir: str, persistence, config,
                         load_all_folds: bool = True, specific_folds: List[int] = None,
                         load_train_data: bool = True, load_test_data: bool = True,
                         max_windows_per_fold: Optional[int] = None) -> ThreeWDataLoader:
    """
