...
def quick_load_cv_data(base_dir: str = None) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """
...
def get_fold_data(cv_folds: List[Dict[str, Any]], fold_number: int) -> Tuple[List, List, List, List]:
    """
...
def print_data_summary(cv_folds: List[Dict[str, Any]], processing_metadata: Dict[str, Any] = None):
    """
    ...
    if processing_metadata:
        print(f"   • Processing config: {processing_metadata.get('processing_config', {}).get('scaling_method', 'unknown')} scaling")
        print(f"   • Window size: {processing_metadata.get('processing_config', {}).get('window_size', 'unknown')}")
        print(f"   • Features: {processing_metadata.get('processing_config', {}).get('target_features', [])}")
...
def training_notebook_setup(base_dir: str = None) -> Dict[str, Any]:
    ...
    augmentor = DataAugmentor(
        random_state=processing_metadata.get('processing_config', {}).get('random_seed', 42),
        verbose=True
    )
...
def example_training_loop(cv_folds: List[Dict[str, Any]]):
    ...
    for fold_num in range(1, len(cv_folds) + 1):
        train_dfs, train_classes, test_dfs, test_classes = get_fold_data(cv_folds, fold_num)
