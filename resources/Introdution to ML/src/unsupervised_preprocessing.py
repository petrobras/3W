...
class UnsupervisedDataPreprocessor:
    ...
    def __init__(self, max_training_samples=1000, max_anomaly_samples=300, random_seed=42):
        """
    ...
    def sample_windows(self, normal_windows, anomaly_windows):
        ...
        if len(normal_windows) > self.max_training_samples:
            print(f"📊 Sampling {self.max_training_samples} normal windows from {len(normal_windows)} available...")
            sampled_indices = random.sample(range(len(normal_windows)), self.max_training_samples)
        ...
        if len(anomaly_windows) > self.max_anomaly_samples:
            print(f"📊 Sampling {self.max_anomaly_samples} anomaly windows from {len(anomaly_windows)} available...")
            sampled_indices = random.sample(range(len(anomaly_windows)), self.max_anomaly_samples)
    ...
    def convert_windows_to_arrays(self, windows, window_type="normal", progress_step=200):
        """
        ...
        for i, window in enumerate(windows):
            if i % progress_step == 0 and i > 0:
                print(f"\r📊 Converting {window_type} windows to arrays... {i}/{len(windows)}", end="")
        ...
        print(f"\r📊 Converting {window_type} windows to arrays... ✅ ({len(arrays)} valid processed)")
    ...
    def validate_and_prepare_arrays(self, normal_arrays, anomaly_arrays):
        ...
        print(f"✅ Valid arrays: {len(valid_normal_arrays)} normal, {len(valid_anomaly_arrays)} anomaly")
        ...
        print(f"   • Normal data range: [{np.min(normal_data):.3f}, {np.max(normal_data):.3f}]")
        print(f"   • Anomaly data range: [{np.min(anomaly_data):.3f}, {np.max(anomaly_data):.3f}]")
    ...
    def apply_stability_scaling(self, normal_data, anomaly_data):
        ...
        print(f"   • Normal data range: [{np.min(normal_scaled):.3f}, {np.max(normal_scaled):.3f}]")
        print(f"   • Anomaly data range: [{np.min(anomaly_scaled):.3f}, {np.max(anomaly_scaled):.3f}]")
    ...
    def prepare_full_pipeline(self, normal_windows, anomaly_windows):
        ...
        sampled_normal, sampled_anomaly = self.sample_windows(normal_windows, anomaly_windows)
        
        # Step 2: Convert to arrays
        normal_arrays = self.convert_windows_to_arrays(sampled_normal, "normal")
        anomaly_arrays = self.convert_windows_to_arrays(sampled_anomaly, "anomaly", progress_step=100)
        
        # Step 3: Validate and prepare
        normal_data, anomaly_data = self.validate_and_prepare_arrays(normal_arrays, anomaly_arrays)
        
        # Step 4: Apply stability scaling
        normal_scaled, anomaly_scaled = self.apply_stability_scaling(normal_data, anomaly_data)
