...
This project adheres to a [code of conduct]\(https\://github.com/petrobras/3W/blob/main/CODE\_OF\_CONDUCT.md) that we expect all contributors to follow. Please be respectful, inclusive, and considerate in all interactions.
...
\- Using prebuilt binary (Linux / macOS):
  ...
  curl -sSL https\://github.com/lycheeverse/lychee/releases/latest/download/lychee-x86\_64-unknown-linux-gnu.tar.gz | tar -xz
...
│   ├── base\_models.py                    # BaseModels (abstract model interface)

│   ├── base\_trainer.py                   # BaseTrainer (training orchestration)

│   ├── base\_dataset.py                   # BaseDataset (data management)

│   ├── base\_preprocessing.py             # BasePreprocessing (transform steps)

│   ├── base\_feature\_extractor.py         # BaseFeatureExtractor (feature engineering)

│   ├── base\_assessment.py                # BaseAssessment (evaluation)

│   ├── base\_pipeline.py                  # BasePipeline (pipeline orchestration)

│   ├── base\_transform.py                 # BaseTransform (transform interface)

│   ├── base\_instantiable.py              # Instantiable (config-driven instantiation)
...
│   ├── sklearn\_models.py                 # Scikit-learn wrappers

│   └── torch\_models.py                   # Additional PyTorch models

├── trainer/

│   ├── sklearn\_trainer.py                # Scikit-learn trainer

│   └── torch\_trainer.py                  # PyTorch trainer
...
class MyComponentConfig(BaseModel, Instantiable):
    ...
    @field\_validator("param1")

    @classmethod

    def validate\_param1(cls, v):
...
class YourModelConfig(ModelsConfig, Instantiable):
    ...
    learning\_rate: float = Field(default=0.01, description="Learning rate", gt=0)
...
class YourModel(BaseModels):
    ...
    def \_\_init\_\_(self, config: YourModelConfig):
...
class YourTorchModel(BaseModels, nn.Module):
    ...
    def load(cls, filename: str | Path) -> "YourTorchModel":
        ...
        instance.load\_state\_dict(torch.load(filename))
...
Add to \`toolkit/ThreeWToolkit/models/\_\_init\_\_.py\`:

\`\`\`python

from .your\_model import YourModel, YourModelConfig
...
Create \`tests/models/test\_your\_model.py\`:
...
class TestYourModel:
    ...
    def test\_initialization(self, model):

        """Test model initialization."""

        assert model.model\_name == "YourModel"

        assert model.config.param1 == 10

    def test\_predict(self, model):
    ...
    def test\_save\_load(self, model, tmp\_path):

        """Test save and load."""

        path = tmp\_path / "model.pkl"

        model.save(path)

        loaded = YourModel.load(path)

        assert loaded.model\_name == model.model\_name
...
class YourPreprocessingConfig(BasePreprocessingConfig, Instantiable):
    ...
    column\_names: list[str] | None = Field(
...
class YourPreprocessing(BasePreprocessing):
    ...
    def \_\_init\_\_(self, config: YourPreprocessingConfig):

        super().\_\_init\_\_(config)

        self.config = config

        self.\_fitted\_stats = None
    ...
    def fit\_and\_transform(
        ...
        signal\_df = dataset.signal\_df

        label\_df = dataset.label\_df

        \# Compute statistics on original data (fit)

        self.\_fitted\_stats = signal\_df.describe()

        \# Create transformed copy (non-destructive)

        transformed\_signal = signal\_df.copy()

        \# Apply transformation...

        return DatasetOutputs(

            signal=transformed\_signal,
...
def test\_your\_preprocessing(dataset):
    ...
    result = preprocessing.fit\_and\_transform(dataset)

    \# Original dataset unchanged

    assert dataset.signal\_df is not result.signal

    \# Result has correct shape

    assert result.signal.shape == dataset.signal\_df.shape
...
class YourFeatureExtractorConfig(BaseModel, Instantiable):
    ...
    window\_size: int = Field(default=100, description="Window size")

    overlap\_offset: int = Field(default=50, description="Overlap offset")
...
class YourFeatureExtractor(
    ...
    def \_\_init\_\_(self, config: YourFeatureExtractorConfig):

        super().\_\_init\_\_(config)

        self.window\_size = config.window\_size

        self.overlap\_offset = config.overlap\_offset

    def extract\_features(self, signal\_df) -> np.ndarray:
...
class YourAssessment(BaseAssessment):
    ...
    def assess(
        ...
        metrics = {
            ...
            "accuracy": np.mean(y\_true == y\_pred),
...
pytest tests/models/test\_your\_model.py

\# Run single test

pytest tests/models/test\_your\_model.py::TestYourModel::test\_predict

\# Run with verbose output

pytest tests/models/test\_your\_model.py -v

\# Run with coverage report

pytest tests/models/test\_your\_model.py --cov=toolkit/ThreeWToolkit/models
...
toolkit/ThreeWToolkit/models/your\_model.py

tests/models/test\_your\_model.py
...
from ThreeWToolkit.utils import general\_utils
...
def fit\_and\_transform(self, dataset):

    return dataset.transform()

\# ✅ Good

def fit\_and\_transform(self, dataset: BaseDataset) -> DatasetOutputs:

    """Fit and transform dataset."""

    return DatasetOutputs(signal=transformed, label=dataset.label\_df)
...
class MyConfig(BaseModel):
    ...
    @field\_validator("learning\_rate")

    @classmethod

    def validate\_lr(cls, v):

        if v > 0.1:

            raise ValueError("learning\_rate should typically be <= 0.1")
