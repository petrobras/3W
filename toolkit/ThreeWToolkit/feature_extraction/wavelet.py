...
class WaveletConfig(BaseFeatureExtractorConfig):
    ...
    wavelet: str = Field(
        default="haar",
        description="Name of the wavelet to use for decomposition. Must be a valid wavelet name supported by PyWavelets.",
    )

    level: int = Field(
        default=7,
        gt=1,
        description="Number of decomposition levels for wavelet transform. Must be a positive integer (>= 1).",
    )

    full: bool = Field(
        default=False,
        description="Whether to return both approximation and detail coefficients or only detail coefficients (False).",
    )

    _target: type = PrivateAttr(default_factory=lambda: WaveletFeatures)
...
class WaveletFeatures(BaseFeatureExtractor):
    ...
    def _initialize_wavelet_filters(self):
        ...
        if self.config.full:
            ...
            self.features = [
                f"{f}{level}"
                for level in range(self.config.level, 0, -1)
                for f in ["A", "D"]
            ] + ["A0"]
            responses = [wave for level in swt_coefficients for wave in level] + [
                impulse
            ]
        else:  # Approximation just for the last level, details for all levels
            self.features = (
                [f"A{self.config.level}"]
                + [f"D{level}" for level in range(self.config.level, 0, -1)]
                + ["A0"]
            )
    ...
    def transform(self, data: DatasetOutputs) -> DatasetOutputs:
        ...
        signal = (
            data.signal.values
        )  # Shape: ((num_windows * num_features), window_size)

        if signal.shape[1] != 2**self.config.level:
            raise ValueError(
                f"Input window size must be 2**level ({2**self.config.level}), but got {signal.shape[1]}."
            )
