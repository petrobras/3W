...
class BaseScoreArgsValidator(BaseModel):
    ...
    def check_shapes(self):
        ...
        if self.sample_weight is not None and len(self.sample_weight) != len(
            self.y_true
        ):
            raise ValueError(
...
class ZeroDivisionValidator(BaseModel):
    ...
    def check_zero_division(cls, v):
        if v not in {"warn", 0, 1}:
            raise ValueError(
                f"'zero_division' must be one of ['warn', 0, 1], got '{v}'"
            )
        return v
...
class AveragePrecisionScoreArgsValidator(
...
class PrecisionScoreArgsValidator(
...
class RecallScoreArgsValidator(
    BaseScoreArgsValidator,
    LabelsValidator,
    PosLabelValidator,
    AverageValidator,
...
class RocAucScoreArgsValidator(
    BaseScoreArgsValidator,
    AverageValidator,
    MaxFprValidator,
    MultiClassValidator,
...
class ExplainedVarianceScoreArgsValidator(
