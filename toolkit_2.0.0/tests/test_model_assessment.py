...
class TestModelAssessmentConfig:
    ...
    def test_invalid_task_type(self):
        ...
        with pytest.raises(
            ValueError, match="Input should be 'classification' or 'regression'"
        ):
    ...
    def test_regression_metrics(self):
        ...
        config = ModelAssessmentConfig(
            metrics=["explained_variance"], task_type=TaskType.REGRESSION
        )
...
class TestModelAssessment:
    ...
    def classification_data(self):
        ...
        X, y = make_classification(
            n_samples=100, n_features=10, n_classes=2, random_state=42, n_redundant=0
        )
        return X, y
    ...
    def test_evaluate_sklearn_classification(
        self, classification_data, temp_output_dir
    ):
    ...
    def test_evaluate_mlp_model(
        self, mock_dataloader, mock_mlp_model, classification_data, temp_output_dir
    ):
    ...
    def test_export_results(
        self, mock_sklearn_model, classification_data, temp_output_dir
    ):
        ...
        config = ModelAssessmentConfig(
            metrics=["accuracy", "f1"], output_dir=temp_output_dir, export_results=True
        )
        ...
        _ = assessor.evaluate(mock_sklearn_model, X_test, y_test)
    ...
    def test_get_metric_method(
        ...
        config = ModelAssessmentConfig(
            metrics=["accuracy", "f1"], output_dir=temp_output_dir, export_results=False
        )
    ...
    def test_map_metrics_for_report(self, temp_output_dir):
        ...
        config = ModelAssessmentConfig(
            metrics=["accuracy", "f1", "explained_variance"], output_dir=temp_output_dir
        )
...
class TestModelAssessmentIntegration:
    ...
    def test_full_classification_pipeline(self, tmp_path):
        ...
        assert all(
            not np.isnan(results["metrics"][metric]) for metric in config.metrics
        )
