...
class NormalizeArgsValidator(BaseModel):
    ...
    def validate_numeric(cls, v):
        if isinstance(v, pd.Series):
            ...
            non_numeric = [col for col in v.columns if not pd.api.types.is_numeric_dtype(v[col])]
            if non_numeric:
                raise TypeError(f"All columns must be numeric. Non-numeric columns: {non_numeric}")
