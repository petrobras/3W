...
class MLPTrainer:
    ...
    def validate_epoch(self, model, loader, criterion):
        ...
        with torch.no_grad():
            for xvalues, yvalues in loader:
                xvalues, yvalues = xvalues.to(self.device).float(), yvalues.to(self.device).long()
    ...
    def train(self, epochs: int = 10):
        ...
        for idx_fold, (train_idx, val_idx) in enumerate(skf.split(range(len(self.train_dataset)), y_train_values), 1):
            ...
            print(f"Fold {idx_fold} - Acurácia de Validação Final: {final_fold_acc:.4f}")
        ...
        print(f"Média da Acurácia de Validação nos {self.nfolds} folds: {mean_val_acc:.4f}")
